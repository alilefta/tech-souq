					...
					<AnimatePresence>
						{query && (
							<motion.div
								...
								animate={{ opacity: 1 }}
								exit={{ opacity: 0 }}
								className="absolute bottom-0 left-0 h-px bg-[#FFB400] shadow-[0_0_10px_#FFB400]"
								style={{ width: "100%" }}
							>
								<motion.div
									animate={{ x: ["-100%", "100%"] }}
									transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
									className="w-1/3 h-full bg-linear-to-r from-transparent via-white/40 to-transparent"
								/>
							</motion.div>
						)}
					</AnimatePresence>
