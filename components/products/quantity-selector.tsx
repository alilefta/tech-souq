...
export function QuantitySelector({ qty, onIncrement, onDecrement, isLoading }: QuantityModuleProps) {
	return (
		<div className="flex items-center h-14 w-full sm:w-fit bg-[#1E293B]/40 border border-white/10 rounded-none overflow-hidden group hover:border-[#FFB400]/20 transition-colors">
			...
			*/}
			<div className="flex-1 flex flex-col items-center justify-center px-8 min-w-20">
				<span className="text-[7px] font-mono text-[#94A3B8] uppercase tracking-[0.3em] mb-0.5 opacity-40">Unit_Count</span>
				<span className="text-[#F5F5F0] font-mono font-black text-sm leading-none">{qty.toString().padStart(2, "0")}</span>
			</div>
